...
export const createProductSchema = z
  .object({
    ...
    price: z.coerce.number({ message: "Price is required" }).min(100, "Price must be at least $1.00"),
    type: z.string({ message: "Type is required" }),
    brand: z.string({ message: "Brand is required" }),
    quantityInStock: z.coerce.number({ message: "Quantity is required" }).min(1, "Quantity must be at least 1"),
