...
export default function ProductForm({ setEditMode, product, refetch, setSelectedProduct }: Props) {
  const {
    ...
    z.input<typeof createProductSchema>, // 👈 Input type before transformation
    unknown,
    z.output<typeof createProductSchema> // 👈 Output type after transformation
  >({
    mode: "onTouched",
    ...
    <Box component={Paper} sx={{ p: 4, maxWidth: "lg", mx: "auto" }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={3}>
          ...
          <Grid size={6}>{data?.brands && <AppSelectInput items={data.brands} control={control} name="brand" label="Brand" />}</Grid>
          ...
          <Grid size={12} display="flex" justifyContent="space-between" alignItems="center">
            ...
            {watchFile ? (
              <img src={(watchFile as File & { preview: string }).preview} alt="preview of image" style={{ maxHeight: 200 }} />
            ) : product?.pictureUrl ? (
              <img src={product.pictureUrl} alt="existing product image" style={{ maxHeight: 200 }} />
            ) : null}
          </Grid>
        </Grid>
